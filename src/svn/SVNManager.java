...
public class SVNManager {
    ...
    public void init() {
        ...
        try {
			myWorkingCopyPath = new java.io.File(".").getCanonicalPath();
		} catch (IOException e1) {
			e1.printStackTrace();
			myWorkingCopyPath = "/Vincent";
		}
    }
    
    public String getMyWorkingCopyPath() {
    	return myWorkingCopyPath;
    }
    
    public void setMyWorkingCopyPath(String newPathString){
    	myWorkingCopyPath = newPathString;
    }
    
    public long checkoutRoot(String[] args) throws SVNException {
        ...
        try {
            repositoryURL = SVNURL.parseURIEncoded("https://aglon-svn01/svn/Mobile/AliceInWonderland/trunk/Rank/Adventures%20in%20Wonderland/SourceCode/code/client");
        } catch (SVNException e) {
            //
        }
        //String name = "ash.svnmobile";
        //String password = "A5h201305vN";
        String name = "michael.sorhaindo";
        String password = "AshGamingPassword1";


        /*
         * That's where a new directory will be created
         */
        SVNURL url = repositoryURL; //.appendPath("localisation", false);
        ...
        myWCEventHandler = new SVNEventHandler();
        ...
        try {
            /*
             * updates working dir to 
             * SVNRevision.HEAD means the latest revision to be checked out. 
             */
            revisionNumber =  update(wcDir, SVNRevision.HEAD, true); // to be changed to true in order to pull device specific versions.
     ...
     */
    @SuppressWarnings("unused")
	private static SVNCommitInfo importDirectory(File localPath, SVNURL dstURL, String commitMessage, SVNDepth depth) throws SVNException{
        /*
         * Returns SVNCommitInfo containing information on the new revision committed 
         * (revision number, etc.) 
         */
        ...
        return ourClientManager.getCommitClient().doImport(localPath, dstURL, commitMessage, properties, ignores, unknownNodes, depth);
	...
	private static long checkout(SVNURL url,
        ...
        if(isRecursive)  return updateClient.doCheckout(url, destPath, revision, revision, SVNDepth.INFINITY, false);
	...
	private static void showInfo(File wcPath, SVNRevision revision, boolean isRecursive) throws SVNException {
         ...
         */
        ourClientManager.getWCClient().doInfo(wcPath, revision, isRecursive, new InfoEventHandler());
    }
     ...
     */
    @SuppressWarnings("unused")
	private static SVNCommitInfo copy(SVNCopySource[] srcURL, SVNURL dstURL,
            boolean isMove, boolean makeParents, boolean failOnExit,
    	...
    	//boolean failOnExit = false;
    	//boolean makeParents
    	//boolean makeParents = false;
    	
        return ourClientManager.getCopyClient().doCopy(srcURL,
                dstURL, isMove, makeParents, failOnExit, commitMessage, revisionProperties);
    }
